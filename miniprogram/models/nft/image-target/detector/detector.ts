...
export class Detector {
  ...
  _combine(prunedExtremas: tf.Tensor<tf.Rank>, extremaAngles: tf.Tensor<tf.Rank>, freakDescriptors: tf.Tensor<tf.Rank>) {
  ...
  _computeExtremaFreak(pyramidImagesT, gaussianNumOctaves, prunedExtremas, prunedExtremasAngles) {
	...
	//   checkout implementation of kernel1
				if (j ===PYRAMID_NUM_SCALES_PER_OCTAVES -1 && i !== gaussianNumOctaves -1) {
					continue;
				}
