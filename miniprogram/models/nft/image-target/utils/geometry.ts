...
const linePointSide = (A: number[], B: number[], C: number[]) => {
  return ((B[0]-A[0])*(C[1]-A[1])-(B[1]-A[1])*(C[0]-A[0]));
}

// srcPoints, dstPoints: array of four elements [x, y]
export const checkFourPointsConsistent = (x1: number[], x2: number[], x3: number[], x4: number[], x1p: number[], x2p: number[], x3p: number[], x4p: number[]) => {
...
export const checkThreePointsConsistent = (x1: number[], x2: number[], x3: number[], x1p: number[], x2p: number[], x3p: number[]) => {
  if ((linePointSide(x1, x2, x3) > 0) !== (linePointSide(x1p, x2p, x3p) > 0)) return false;
  return true;
}
...
export const smallestTriangleArea = (x1: any, x2: any, x3: any, x4: any) => {
...
const _vector = (a: number[], b: number[]) => {
  return [
    a[0] - b[0],
    a[1] - b[1]
  ]
...
const _areaOfTriangle = (u: number[], v: number[]) => {
